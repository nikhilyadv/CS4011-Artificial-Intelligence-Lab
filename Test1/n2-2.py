...
class puzzle:                       #CLASS
    ...
    def manhattan(self,item,p1,p2):                     #function to calculater manhattan distance
        for i in range(0,self.n):
            for j in range(0,self.n):
                if item == self.goal[i][j]:
                    return abs(i-p1)+abs(j-p2)
        ...
        for i in range(0,self.n):
            for j in range(0,self.n):
                if self.a[i][j] != 0 and self.a[i][j] != self.goal[i][j]:
                    distance = distance + self.manhattan(self.a[i][j],i,j)
                if self.a[i][j] == 0 and self.goal[i][j] != 0 and flag == 0:
                    flag = 1
                    distance = distance + min((abs(self.n - 1 - i)+abs(self.n - 1 - j)),(abs(self.n - 1 - i) + abs(self.n - 2 - j)))
                if self.a[i][j] == 0 and self.goal[i][j] != 0 and flag == 1:
                    distance = distance + max((abs(self.n - 1 - i)+abs(self.n - 1 - j)),(abs(self.n - 1 - i) + abs(self.n - 2 - j)))
        return distance
